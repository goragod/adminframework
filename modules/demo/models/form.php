        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            try {
                ...
                $save = array(
                    ...
                    'from_time' => $request->post('register_from_time')->date(),
                    ...
                    'color' => $request->post('register_color', array())->filter('0-9A-Z\#'),
                    ...
                    // true หรือ false
                    'social' => $request->post('register_social', array())->toBoolean(),
                    // รับค่าจาก CKEditor
                    'detail' => $request->post('register_detail')->detail(),
                    /* รับค่าจาก province ตัวเลขเท่านั้น เป็นแอเรย์ ไม่ระบุ คืนค่า 0 */
                    'province' => $request->post('register_province', array())->toInt(),
                    'inputgroups' => $request->post('register_inputgroups', array())->toInt(),
                    /* รับค่าจาก province ตัวเลขเท่านั้น เป็นแอเรย์ ยอมรับค่าว่าง */
                    'select.checkbox' => $request->post('select_checkbox', array())->number(),
